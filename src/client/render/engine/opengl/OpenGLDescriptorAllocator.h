...
namespace voxel_game::client::render::engine::opengl {
	...
	public:
		explicit OpenGLDescriptorAllocator(uint32_t maxSets, const std::vector<DescriptorType>& descriptorBindings);

		void clearDescriptors() override;

		std::unique_ptr<DescriptorSet> allocate() override;

		[[nodiscard]] const std::vector<DescriptorType>& getDescriptorBindings() const {
	...
	private:
		...
		std::vector<DescriptorType> mDescriptorBindings;
	};
}
