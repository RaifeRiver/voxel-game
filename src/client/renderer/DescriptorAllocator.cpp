...
namespace voxel_game::client::renderer {
	void DescriptorAllocator::init(const VkDevice device, const uint32_t maxSets, const std::span<DescriptorPoolSizeRatio> poolSizes) {
		std::vector<VkDescriptorPoolSize> poolSizeVector;
		for (DescriptorPoolSizeRatio ratio: poolSizes) {
			poolSizeVector.push_back({ratio.type, static_cast<uint32_t>(ratio.ratio * static_cast<float>(maxSets))});
		}
	...
	void DescriptorAllocator::clear(const VkDevice device) const {
		VK_CHECK(vkResetDescriptorPool(device, mPool, 0));
	}
